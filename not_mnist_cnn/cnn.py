...
with tf.name_scope('weights'):
    weights = {'w_conv1': tf.Variable(tf.random_normal([path_size, path_size, num_channnels, depth], stddev=0.1)),
               ...
               'w_conv3': tf.Variable(tf.random_normal([path_size,path_size,64,128],stddev=0.1)),
               'w_conv4': tf.Variable(tf.random_normal([path_size,path_size,128,128],stddev=0.1)),
               'w_fc1': tf.Variable(tf.random_normal([4 * 4 * 128, 128], stddev=0.1)),
               'w_fc2': tf.Variable(tf.random_normal([128, 128], stddev=0.1)),
               'out': tf.Variable(tf.random_normal([128, num_labels], stddev=0.1))}
with tf.name_scope('biases'):
    biases = {
        'b_conv1': tf.Variable(tf.constant(1.0, shape=[32])),
        'b_conv2': tf.Variable(tf.constant(1.0, shape=[64])),
        'b_conv3': tf.Variable(tf.constant(1.0,shape=[128])),
        'b_conv4': tf.Variable(tf.constant(1.0, shape=[128])),
        'b_fc1': tf.Variable(tf.constant(1.0, shape=[128])),
        'b_fc2': tf.Variable(tf.constant(1.0, shape=[128])),
...
def cnn(data):
    ...
    with tf.name_scope('conv3'):
        conv3 = tf.nn.relu(tf.nn.conv2d(conv2,weights['w_conv3'],[1,1,1,1],padding='SAME',use_cudnn_on_gpu=False,name='conv3')+
                           biases['b_conv3'])

    # print("conv2", conv2.shape)
    with tf.name_scope('pool2'):
        pool2 = tf.nn.max_pool(conv3, [1, 2, 2, 1], [1, 2, 2, 1], padding="SAME", name="pool2")

    with tf.name_scope('conv4'):
        conv4 = tf.nn.relu(tf.nn.conv2d(pool2,weights['w_conv4'],[1,1,1,1],padding='SAME',use_cudnn_on_gpu=False,name='conv4')+biases['b_conv3'])

    with tf.name_scope('pool3'):
        pool3 = tf.nn.max_pool(conv4,[1,2,2,1],[1,2,2,1],padding='SAME',name="pool3")

    # print("pool2", pool2.shape)

    # print("reshape", tf.reshape(pool2, [-1, 7 * 7 * 64]).shape)
    with tf.name_scope('fc_1'):
        fc_1 = tf.nn.relu(tf.matmul(tf.reshape(pool3, [-1, 4 * 4 * 128]), weights['w_fc1']) + biases['b_fc1'])
...
with tf.name_scope('training'):
    ...
    learning_rate = tf.train.exponential_decay(starter_learning_rate, global_step, 10000, 0.5, staircase=True)
